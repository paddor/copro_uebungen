...
public class VariablenMitTyp {
	 ...
	 */
	private final String KWcode = "xileweRWve";
	 ...
	 */
	private void allocate(String name, String value) {
		System.out.println(name + ":"+TAB+"DC "+ value);
	}
	...
	private void alloc() {
		...
		checkDup(lookValue);
		addEntry(lookValue, 'v');
		allocate(lookValue, "0");
		next();
	}
	...
	public static void main(String[] args) throws IOException {
		System.out.println("Enter multiple character keywords, single character variables based program");
		VariablenMitTyp vmt = new VariablenMitTyp("Test_VMT");
//		VariablenMitTyp vnt = new VariablenMitTyp(System.in);
		/*
		 * initialize: rest (see constructor)
		 */
		vmt.next();
		//
		vmt.matchString("PROGRAM");
		vmt.semi();
		vmt.header();
		vmt.topDecls();
		vmt.matchString("BEGIN");
		vmt.prolog();
		vmt.block();
		vmt.matchString("END");
		vmt.epilog();
